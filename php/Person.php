...
class Person{
    ...
    function editPerson($id,$fName,$lName,$user,$pass,$email,$admission,$gender){
        $connection = new Connection();
        $conn = $connection->getConnection();
        $activeUser =$_SESSION["rowUser"];
        $query = "update person
                  set first_name = '$fName', last_name = '$lName', username = '$user', pass ='$pass', email = '$email', admission_date = '$admission', gender = '$gender'
                  where id_person = '$id'";
        $result = pg_query($conn,$query);
...
if($_REQUEST['action'] == 'edit'){
    $person->editPerson($_REQUEST['id'],$_REQUEST['fName'], $_REQUEST['lName'],$_REQUEST['user'],md5($_REQUEST['pass']),$_REQUEST['email'],$_REQUEST['admission'],$_REQUEST['gender']);
    //$person->getPersons();
}
